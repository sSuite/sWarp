...
public class InfoCommand extends AbstractCommand {
	...
	public boolean onExecute(CommandSender sender, String[] args) {
		...
		try {
			...
			sender.sendMessage(ChatColor.GOLD + "Warp information for " + ChatColor.AQUA + targetWarp.getName() + ChatColor.GOLD + ":");
			sender.sendMessage(ChatColor.YELLOW + "Owner: " + ChatColor.RESET + targetWarp.getOwner().getName());
			sender.sendMessage(ChatColor.YELLOW + "Warp type:" + ChatColor.RESET + (targetWarp.isPublic() ? "Public" : "Private"));
			
			String invitees = "";
			OfflinePlayer[] invitedPlayers = targetWarp.getInvitedPlayers();
			
			for (int i = 0; i < invitedPlayers.length; i++) {
				invitees += ChatColor.GOLD + invitedPlayers[i].getName() + ChatColor.RESET + (i + 1 == invitedPlayers.length ? "" : ", ");
			}
			...
			sender.sendMessage(ChatColor.YELLOW + "    World: " + ChatColor.RESET + targetWarp.getLocation().getWorld().getName());
			...
			sender.sendMessage(ChatColor.YELLOW + "    Pitch: " + ChatColor.RESET + targetWarp.getLocation().getPitch());
